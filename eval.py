...
COLORS = {"test": "#c0392b", "baseline": "#2980b9"}
...
def create_report(test, baseline, sample_rollouts, costs, num_segs):
  ...
  <html>
    ...
    res.append(
        "<h1 style='font-size: 50px; font-weight: 700; text-align: center'>Comma Controls Challenge: Report</h1>"
    )
    res.append(
        f"<h3 style='font-size: 30px;'><span style='background: {COLORS['test']}; color: #fff; padding: 10px'>Test Controller: {test}</span> ⚔️ <span style='background: {COLORS['baseline']}; color: #fff; padding: 10px'>Baseline Controller: {baseline}</span></h3>"
    )

    res.append(
        f"<h2 style='font-size: 30px; margin-top: 50px'>Aggregate Costs (total rollouts: {num_segs})</h2>"
    )
    ...
    for ax, cost in zip(axs, ["lataccel_cost", "jerk_cost", "total_cost"]):
        for controller in ["test", "baseline"]:
            ax.hist(
                res_df[res_df["controller"] == controller][cost],
                bins=bins,
                label=controller,
                alpha=0.5,
        ...
        ax.legend()
    res.append(
        f'<img style="max-width:100%" src="data:image/png;base64,{img2base64(fig)}" alt="Plot">'
    )
    agg_df = (
        res_df.groupby("controller")
        .agg({"lataccel_cost": "mean", "jerk_cost": "mean", "total_cost": "mean"})
        .round(3)
        .reset_index()
    )
    ...
    if passed_baseline:
        res.append(
            f"<h3 style='font-size: 20px; color: #27ae60'> ✅ Test Controller ({test}) passed Baseline Controller ({baseline})! ✅ </h3>"
        )
        res.append(
            """<p>Check the leaderboard
      ...
      !</p>"""
        ...
        res.append(
            f"<h3 style='font-size: 20px; color: #c0392b'> ❌ Test Controller ({test}) failed to beat Baseline Controller ({baseline})! ❌</h3>"
        )
    ...
    fig, axs = plt.subplots(
        ncols=1, nrows=SAMPLE_ROLLOUTS, figsize=(15, 3 * SAMPLE_ROLLOUTS), sharex=True
    )
    for ax, rollout in zip(axs, sample_rollouts):
        ax.plot(
            rollout["desired_lataccel"],
            label="Desired Lateral Acceleration",
            color="#27ae60",
        )
        ax.plot(
            rollout["test_controller_lataccel"],
            label="Test Controller Lateral Acceleration",
            color=COLORS["test"],
        )
        ax.plot(
            rollout["baseline_controller_lataccel"],
            label="Baseline Controller Lateral Acceleration",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_controller", default="pid", choices=available_controllers
    )
    parser.add_argument(
        "--baseline_controller", default="pid", choices=available_controllers
    )
    ...
    for d, data_file in enumerate(tqdm(files[:SAMPLE_ROLLOUTS], total=SAMPLE_ROLLOUTS)):
        test_cost, test_target_lataccel, test_current_lataccel = run_rollout(
            data_file, args.test_controller, args.model_path, debug=False
        )
        (
            baseline_cost,
            baseline_target_lataccel,
            baseline_current_lataccel,
        ) = run_rollout(
            data_file, args.baseline_controller, args.model_path, debug=False
        )
    ...
    for controller_cat, controller_type in [
        ("baseline", args.baseline_controller),
        ("test", args.test_controller),
    ]:
        print(
            f"Running batch rollouts => {controller_cat} controller: {controller_type}"
        )
        ...
        results = process_map(
            rollout_partial, files[SAMPLE_ROLLOUTS:], max_workers=16, chunksize=10
        )
